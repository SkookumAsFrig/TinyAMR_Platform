...
//    frequency = 0;
  ...
  long Top = (system_clock /( abs(frequency) * 2 * prescaler)) - 1;   // TOP  frequency=(system clock)/ [2 * prescaler * (TOP +1)]
  if (Top>65535)
    Top = 65535;
...
void loop() {   
  ...
  if(Serial.available()>0){
    ...
    data[0] = (((int)buff[0]) << 9) | (((int)buff[1] & 0x7F) << 2);
    data[0] = data[0]/4;
    data[1] = (((int)buff[2]) << 9) | (((int)buff[3] & 0x7F) << 2);
    data[1] = data[1]/4;
    data[2] = (((int)buff[4]) << 9) | (((int)buff[5] & 0x7F) << 2);
    data[2] = data[2]/4;
    data[3] = (((int)buff[6]) << 9) | (((int)buff[7] & 0x7F) << 2);
    data[3] = data[3]/4;
    data[4] = buff[8];
    digitalWrite(12, LOW);
